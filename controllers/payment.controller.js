...
async function createPaymentMidtrans(bookingId, paymentMethod) {
  try {
    ...
    const parameter = {
      ...
      callbacks: {
        finish: `${CLIENT_BASE_URL}/success?booking_code=${checkBook.id}`,
        error: `${CLIENT_BASE_URL}/error?booking_code=${checkBook.id}`,
        pending: `${CLIENT_BASE_URL}/riwayat-pemesanan?booking_code=${checkBook.id}`,
      },
    };

    const authString = Buffer.from(`${isProduction ? PAYMENT_PROD_SERVER_KEY : PAYMENT_DEV_SERVER_KEY}:`).toString('base64');

    const response = await fetch(
      `https://app.${isProduction ? 'midtrans' : 'sandbox.midtrans'}.com/snap/v1/transactions`,
      {
        method: "POST",
        headers: {
